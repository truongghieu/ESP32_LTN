...
uint8_t bitInx = 7;
	...
	uSec = getSignalLevel( 57, 0 );
//	ESP_LOGI( TAG, "Response = %d", uSec );
	if( uSec<0 ) return DHT_TIMEOUT_ERROR; 

	// -- 80us up ------------------------

	uSec = getSignalLevel( 83, 1 );
//	ESP_LOGI( TAG, "Response = %d", uSec );
	if( uSec<0 ) return DHT_TIMEOUT_ERROR;
    ...
    int x=dhtData[0];
	int y=dhtData[1];
	int phan_nguyen = x + (y / 10);
float phan_thap_phan = (float)(y % 10) / 10;
float ket_qua = phan_nguyen + phan_thap_phan;
humidity=ket_qua;
...
*/
	...
	int x_1=dhtData[2];
	int y_1=dhtData[3];
	int phan_nguyen_1 = x_1 + (y_1 / 10);
float phan_thap_phan_1 = (float)(y_1 % 10) / 10;
float ket_qua_1 = phan_nguyen_1 + phan_thap_phan_1;
temperature=ket_qua_1;
